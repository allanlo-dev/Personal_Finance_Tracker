...
def generate_mosaic_chart(totals: dict[str, float], incometotals: dict[str, float], expensetotals: dict[str, float]) -> None:
    ...
    """
    colors = [
        "#2DBDB1" if t == "Income" else "#D23917" for t in totals.keys()
    ]
...
def get_charts_data(trans: list[Transaction]) -> tuple[dict[str, float], dict[str, float], dict[str, float]]:
    transactions = trans
